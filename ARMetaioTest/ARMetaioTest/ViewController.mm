...
- (void)onTrackingEvent:(const metaio::stlcompat::Vector<metaio::TrackingValues> &)poses{
    ...
    if(poses[0].quality >= 0.5){
        ...
        //getTrackingValues必要？
        //auto trackingValue = m_metaioSDK->getTrackingValues(true);
        
        //3次元 → 2次元
        //self.arview.arLocation = m_metaioSDK->getScreenCoordinatesFrom3DPosition(poses[0].coordinateSystemID, self.arview.transComp);
        //self.arview.arLocation = m_metaioSDK->getViewportCoordinatesFrom3DPosition(poses[0].coordinateSystemID, self.arview.transComp, true);
        //self.arview.arLocation = m_metaioSDK->getViewportCoordinatesFrom3DPosition(poses[0].coordinateSystemID, poses[0].translation, true);
